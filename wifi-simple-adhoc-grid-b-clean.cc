/*
 * wifi-simple-adhoc-grid-ac-clean.cc
 *
...
NS_LOG_COMPONENT_DEFINE ("wifi-simple-adhoc-grid-b-clean");
...
void ComputePDR_and_CF ( double _simulationTime )
{
	...
	if (Simulator::Now ().GetSeconds()<_simulationTime){ // Metodo recursivo
		...
		std::cout << "\n"<< Simulator::Now ().GetMilliSeconds() << " Seed: " << numSeeds << " Mode: " << mode << " Topology: " << topology <<std::endl;
...
void Predict (double _simulationTime)
{
	if (Simulator::Now ().GetSeconds()<_simulationTime){ // Metodo recursivo
		...
		for (std::map<std::pair<uint32_t, uint32_t>,double>::const_iterator iterator = m_nodePDR.begin();
			...
			if (mode >= 12.0) {
				m_predict[std::make_pair(_Node_, _App_)] = apb.Func(_Node_, _nReps_, _cbr_, _pdr_, _cf_, _bitr_, _ql_);
			}*/
			if (mode >= 6.0 && mode < 12.0) {
				m_predict[std::make_pair(_Node_, _App_)] = apb.Func(_Node_, _App_, _cbr_, _pdr_, _cf_, _bitr_, _ql_);
			}
			else {
				m_predict[std::make_pair(_Node_, _App_)] = apb.Func(_Node_, _App_, _cbr_, _pdrEWMA_, _cfEWMA_, _bitr_, _ql_);
			}
...
int main (int argc, char *argv[])
{
	...
	cmd.AddValue ("training", "simulation in training mode", TrainingMode);
